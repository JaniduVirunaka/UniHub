    ...
    <PageWrapper title="Campus Sports" subtitle="Explore the sports teams available at UniHub.">
      ...
      <div className="mb-6 flex items-center justify-between gap-4 rounded-2xl border border-emerald-400/30 bg-emerald-400/10 px-5 py-4">
        ...
        <div className="flex shrink-0 gap-2">
          <Link to="/login"  className="rounded-full bg-emerald-400 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-emerald-300">Sign In</Link>
          <Link to="/signup" className="rounded-full border border-white/20 px-4 py-2 text-sm font-medium text-slate-200 transition hover:bg-white/10">Sign Up</Link>
        </div>
      </div>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {sports.map(sport => (
            <GlassCard key={sport._id} className="transition hover:-translate-y-1 hover:bg-white/10">
              ...
              <Link
                to="/login"
                className="mt-6 inline-flex items-center gap-2 rounded-2xl border border-white/20 px-4 py-3 text-sm font-semibold text-slate-300 transition hover:bg-white/10"
              >
                <LogIn size={16} />
                Sign in to join
              </Link>
