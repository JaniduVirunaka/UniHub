...
function ViceCaptainRequests() {
  ...
  if (loading) return <PageWrapper title="Vice Captain Requests"><LoadingSpinner text="Loading requests..." /></PageWrapper>;

  if (!sport) return (
    <PageWrapper title="Vice Captain Requests" subtitle="No sport assigned to this vice captain yet.">
      <GlassCard><p className="text-slate-300">No sport assigned to you as vice captain.</p></GlassCard>
    </PageWrapper>
  );

  return (
    <PageWrapper title="Vice Captain Requests" subtitle={`Managing requests for: ${sport.name}`}>
      {requests.length === 0 ? (
